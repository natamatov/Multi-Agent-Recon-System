...
class SecurityTasks:
    ...
    def correlate_vulnerabilities_task(self, agent) -> Task:
        return Task(
            description=dedent("""
                ...
                найдите потенциальные уязвимости (CVE) и эксплойты. Для каждой технологии или сервиса с известной
                версией:
                1. Укажите идентификаторы возможных CVE.
                2. Используйте инструмент Pompem для поиска доступных эксплойтов (PoC).
                3. Оцените критичность по шкале CVSS (Critical, High, Medium, Low).
                4. Дайте краткое техническое описание вектора атаки для каждой CVE.
    ...
    def exploit_verification_task(self, agent) -> Task:
        return Task(
            description=dedent("""
                На основе данных об эксплойтах, полученных от Threat Intel агента, выполните ПОЛНЫЙ ЦИКЛ верификации:
                1. Выберите наиболее подходящий эксплойт (PoC) для обнаруженных технологий.
                2. Загрузите код эксплойта, используя инструмент pompem_exploit_download.
                3. Автоматически определите необходимые зависимости (библиотеки Python, системные пакеты) 
                   и установите их через инструмент install_exploit_dependencies.
                4. Сформируйте точную команду для запуска и выполните её через execute_exploit_payload 
                   против целевой системы (<TARGET>).
                5. Оцените результат выполнения: удалось ли подтвердить уязвимость (получить ответ от сервера, 
                   считать баннер, выполнить тестовую команду).
                
                ВАЖНО: Выполняйте запуск только если уверены в безопасности PoC. Ваш отчет должен содержать 
                логи всех этапов: установки, запуска и финальный вердикт.
            """),
            expected_output="Технический отчет с путями к загруженным эксплойтам и командами для верификации.",
            agent=agent,
        )
